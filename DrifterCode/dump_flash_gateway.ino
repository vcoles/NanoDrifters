//Gateway modified to ask node to dump its flash to gateway
//This code does not activate GPS, SD or SPI Flash only RF69 and Software Serial
//
...
uint8_t nodeToDump=117;
uint8_t nodeBroadcast = 255;
...
//  digitalWrite(CS_flash, HIGH);
  ...
  Serial.print("NODEID as DUMP node is ");
  Serial.println(NODEID,DEC);
  Serial.print("Node to Dump is "); Serial.println(nodeToDump);
  delay(10);
...
Serial.println("Enter nodeToDump as 3I, i.e 115");
char nodebuf[4];
while(Serial.available() < 3) {}  // wait for 3 characters to be typed in
  for(int i = 0; i <=2; i++){     // read them and convert to uint8_t
    nodebuf[i] =Serial.read();
    nodebuf[i+1]='\0';
}
nodeToDump = atoi(nodebuf);
Serial.print(nodeToDump); Serial.print(" = "); Serial.println((char)nodeToDump);

//digitalWrite(CS_radio, LOW);       
//radio.sendWithRetry(nodeToDump, "d", 1);  //send "d" character to nodeToDump
//Serial.print("Sent d to node ");Serial.println(nodeToDump);
...
void loop() {
  ...
  if(input == 's') {
    digitalWrite(CS_radio, LOW);       
    radio.sendWithRetry(nodeToDump, "s", 1);  //send "d" character to nodeToDump
    Serial.print("Sent STOP (s) to node ");Serial.println(nodeToDump);
  }
  if(input == 'S') {
    digitalWrite(CS_radio, LOW);       
    radio.sendWithRetry(nodeBroadcast, "s", 1);  //send "s" character to all nodes
    Serial.println("Sent STOP (s) to all nodes ");
  }
  if(input == 'g') {
    digitalWrite(CS_radio, LOW);       
    radio.sendWithRetry(nodeToDump, "g", 1);  //send "g" character to nodeToDump
    Serial.print("Sent GO (g) to node ");Serial.println(nodeToDump);
  }
  if(input == 'G') {
    digitalWrite(CS_radio, LOW);       
    radio.sendWithRetry(nodeBroadcast, "g", 1);  //send "g" character to all nodes
    Serial.println("Sent GO (g) to all nodes ");
  }
...
//      Serial.println(" - ACK sent.");
    ...
    //File dataFile = SD.open("data" + String(senderID) + ".txt", FILE_WRITE);
    File dataFile = SD.open("gateway.txt", FILE_WRITE); //use same file for node data  
    //File dataFile = SD.open("test" + String(senderID) + ".txt", FILE_WRITE);
//    Serial.println("SDopen");
    if (dataFile){
